...
public class Field {
  ...
  public Field(int size, int[] values) {
    ...
    for (int i = 0; i < values.length; ++i){ cells[i] = new Cell(size, i, values[i], values[i] != 0); }
    
    // compute the starting indices for all the rows and columns
    for (int i = 0; i < size * size; ++i) {
      if (i % size == 0) { rows.add(new Row(size, cells, i)); }
      if (i < size) { columns.add(new Column(size, cells, i)); }
    }
